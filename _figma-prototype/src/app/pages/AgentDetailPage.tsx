...
import { parseBookString } from '../types';
...
export const AgentDetailPage = ({ agents }: { agents: Agent[] }) => {
  ...
  if (!agent) {
    return (
      <div className="flex flex-col items-center justify-center h-64 text-center">
        ...
        <p className="text-gray-500 mb-6">Der gesuchte KI-Agent existiert nicht.</p>
      ...
      <Tabs.Root defaultValue="overview" className="flex flex-col space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8 min-h-[400px]">
          ...
          <Tabs.Content value="primary-books" className="space-y-8 outline-none animate-in slide-in-from-bottom-2 duration-300">
            <div>
              <h3 className="text-xl font-bold text-gray-900 mb-6 border-l-4 border-indigo-500 pl-4">Primärliteratur</h3>
          ...
          <Tabs.Content value="secondary-books" className="space-y-8 outline-none animate-in slide-in-from-bottom-2 duration-300">
            <div>
              <h3 className="text-xl font-bold text-gray-900 mb-6 border-l-4 border-slate-400 pl-4">Sekundärliteratur</h3>
          ...
          <Tabs.Content value="essays" className="outline-none animate-in slide-in-from-bottom-2 duration-300">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {agent.essays.map((essay, idx) => (
                <a
                  key={idx}
                  href={`${import.meta.env.BASE_URL}assistants/${encodeURIComponent(agent.id)}/essays/${encodeURIComponent(essay)}`}
                  target="_blank"
                  rel="noreferrer"
          ...
          <Tabs.Content value="concepts" className="outline-none animate-in slide-in-from-bottom-2 duration-300">
            ...
            <div className="space-y-3">
              {agent.concepts.map((concept, idx) => (
                <a
                  key={idx}
                  href={`${import.meta.env.BASE_URL}assistants/${encodeURIComponent(agent.id)}/concepts/${encodeURIComponent(concept)}`}
                  target="_blank"
                  rel="noreferrer"
          ...
          <Tabs.Content value="quotes" className="outline-none animate-in slide-in-from-bottom-2 duration-300">
             ...
             <div className="grid gap-6">
                {agent.quotes?.map((quote, idx) => (
                  <blockquote key={idx} className="relative p-6 bg-amber-50 rounded-xl border-l-4 border-amber-400 shadow-sm">
          ...
          <Tabs.Content value="taxonomies" className="outline-none animate-in slide-in-from-bottom-2 duration-300">
            ...
            <div className="space-y-2">
               {agent.taxonomies?.map((tax, idx) => (
                 <div key={idx} className="flex items-center gap-2 text-gray-700 bg-white border border-gray-200 p-3 rounded-lg shadow-sm">
           ...
           <Tabs.Content value="conversations" className="outline-none animate-in slide-in-from-bottom-2 duration-300">
            ...
            <div className="space-y-4">
              {agent.conversations?.map((conv) => (
                <div key={conv.id} className="p-5 rounded-xl border border-gray-200 hover:shadow-md transition-shadow bg-white">
...
const TabTrigger = ({ value, icon, label }: { value: string, icon: ReactNode, label: string }) => {
  return (
    <Tabs.Trigger 
      value={value}
      className={clsx(
...
const BookList = ({ books, isSecondary = false }: { books: string[], isSecondary?: boolean }) => {
    ...
    <div className="grid grid-cols-1 gap-4">
      {books.map((bookStr, idx) => {
        ...
        return (
          <a
            key={idx}
            href={`${import.meta.env.BASE_URL}books/${encodeURIComponent(bookStr)}/index.html`}
            target="_blank"
            rel="noreferrer"
          ...
          >
            <div className={clsx("p-3 rounded-lg flex-shrink-0", isSecondary ? "bg-slate-200 text-slate-600" : "bg-indigo-100 text-indigo-600")}>
              <Book size={24} />
            </div>
            <div className="flex-1 min-w-0">
              <h4 className="font-bold text-gray-900 truncate pr-4" title={book.title}>{book.title}</h4>
