...
const RED = "\x1b[31m";
const BRIGHT_GREEN = "\x1b[92m"; // GA-Nummer eingefügt (Eintrag hatte keine ga-Zeile)
const DARK_GREEN = "\x1b[32m"; // GA-Nummer ergänzt (an bestehende ga-Zeile angehängt)
...
const HELP_TEXT = `Ergänzt fehlende GA-Nummern in rudolf-steiner-ga-vortrag-verzeichnis.yaml.
...
Verwendung:
  node add_ga_from_ga_pdf_inhaltsverzeichnis.mjs [Optionen]

Optionen:
  --help, -h     Diese Hilfe anzeigen
  --write        Änderungen in die YAML-Datei schreiben (ohne: nur Vorschau)
  --range RANGE  Nur bestimmte GA-Bände verarbeiten
                 Beispiele: 51,52,53  oder  332-337  oder  68a,68b  oder  68a-68c
...
function main() {
  ...
  for (let idx = 0; idx < total; idx++) {
    ...
    for (const lecture of lectures) {
      ...
      if (withoutGa.length === 0 && withGaButNotOurs.length === 0) {
        ...
        const msg =
          entries.length === 0
            ...
            : "kein Eintrag ohne GA-Nummer gefunden";
        console.error(`${RED}${prefix}Fehler: GA ${gaNum}, Datum ${display} (${id}) ${datum} – ${msg}.${RESET}`);
        if (sourceLine) {
      ...
      const prefix = lineId ? `[${lineId}] ` : "";
      console.log("");
      const color = isAppend ? DARK_GREEN : BRIGHT_GREEN;
      const action = isAppend ? (doWrite ? "ergänzt" : "würde ergänzt") : (doWrite ? "eingefügt" : "würde eingefügt");
      console.log(
        `${color}  ${prefix}GA-Nummer ${action}: ga: ${gaNum} für ${display} (${id})${RESET}`
      );
      if (sourceLine) {
        console.log(`${GRAY}  Zeile: ${sourceLine}${RESET}`);
      }
